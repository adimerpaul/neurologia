<template>
  <q-footer>
    <div class="bg-grey-3">
      <div class="row q-pa-lg">
        <div class="col-6 col-md-3 flex flex-center">
          <q-btn type="a" target="_blank" href="https://www.facebook.com/profile.php?id=100068249745954" flat dense
                 size="22px" color="primary" class="q-ma-md"
          >
            <div class="row items-center no-wrap">
              <q-icon left name="fa-brands fa-facebook" />
              <div class="text-left text-grey text-subtitle2 text-bold " style="line-height: 1.2;">
                Facebook<br>5ta. Jornada Nacional de Neurología
              </div>
            </div>
          </q-btn>
        </div>
        <div class="col-6 col-md-3 flex flex-center">
          <q-img src="/images/mejorado.png" width="100px" class="cursor-pointer"
                 @click="$router.push('/')"
          />
        </div>
        <div class="col-6 col-md-3">
<!--          <div class="col-6 col-md-3 q-pa-md">-->
            <div class=" text-grey text-bold text-center">5𝐭𝗮 𝗝𝗼𝗿𝗻𝗮𝗱𝗮 𝗡𝗮𝗰𝗶𝗼𝗻𝗮𝗹 𝗱𝗲 𝗡𝗲𝘂𝗿𝗼𝗹𝗼𝗴𝗶́𝗮</div>
            <q-separator />
            <div class="text-grey text-center">
              "Avances en diagnóstico y tratamiento de enfermedades Neurológicas"
            </div>
            <div class="text-grey text-center"> 12 , 13 𝕪 14 de Junio 2025</div>
<!--          </div>-->
<!--          <div class="col-6 col-md-3 q-pa-md">-->
<!--          </div>-->
        </div>
        <div class="col-6 col-md-3">
          <div class=" text-grey text-bold text-center">
<!--            1𝐫𝐞 𝗖𝘂𝗿𝘀𝗼 𝗧𝗮𝗹𝗹𝗲𝗿 𝗱𝗲 𝗘𝘅𝗽𝗹𝗼𝗿𝗮𝗰𝗶𝗼́𝗻 neur𝗼𝗹𝗼𝗴𝗶𝗰𝗮-->
            1𝐫𝐞 𝗖𝘂𝗿𝘀𝗼 𝗧𝗮𝗹𝗹𝗲𝗿 𝗱𝗲 𝗘𝘅𝗽𝗹𝗼𝗿𝗮𝗰𝗶𝗼́𝗻 Neurológica
          </div>
          <q-separator />
<!--          <div class="text-grey">Emergencias Neurológicas</div>-->
          <div class="text-grey text-center">13  de Junio</div>
        </div>
<!--        <div class="col-12">-->
<!--          <div class="row">-->
<!--            <div class="col-12 col-md-1"></div>-->
<!--            <div class="col-12 col-md-3 flex flex-center">-->
<!--              <div class="text-red-6 text-bold text-h5">-->
<!--                <q-img src="/logo.png" width="100px" class="cursor-pointer"-->
<!--                       @click="$router.push('/')"-->
<!--                />-->
<!--              </div>-->
<!--            </div>-->
<!--            <div class="col-6 col-md-3 q-pa-md">-->
<!--              <div class=" text-grey text-bold">𝟒𝐭𝗮 𝗝𝗼𝗿𝗻𝗮𝗱𝗮 𝗡𝗮𝗰𝗶𝗼𝗻𝗮𝗹 𝗱𝗲 𝗡𝗲𝘂𝗿𝗼𝗹𝗼𝗴𝗶́𝗮</div>-->
<!--              <q-separator />-->
<!--              <div class="text-grey">Actualizaciones en Enfermedades Neurológica</div>-->
<!--              <div class="text-grey">𝕕í𝕒𝕤 𝟙𝟛 , 𝟙𝟜 𝕪 𝟙𝟝 𝕕𝕖 𝕤𝕖𝕡𝕥𝕚𝕖𝕞𝕓𝕣𝕖</div>-->
<!--            </div>-->
<!--            <div class="col-6 col-md-3 q-pa-md">-->
<!--              <div class=" text-grey text-bold">𝟐𝐝𝐨 𝗦𝗶𝗺𝗽𝗼𝘀𝗶𝗼 𝗜𝗻𝘁𝗲𝗿𝗻𝗮𝗰𝗶𝗼𝗻𝗮𝗹</div>-->
<!--              <q-separator />-->
<!--              <div class="text-grey">Emergencias Neurológicas</div>-->
<!--              <div class="text-grey">𝕕í𝕒𝕤 𝟙𝟞 𝕪 𝟙𝟟 𝕕𝕖 𝕤𝕖𝕡𝕥𝕚𝕖𝕞𝕓𝕣𝕖</div>-->
<!--            </div>-->
<!--            <div class="col-12 col-md-1"></div>-->
<!--          </div>-->
<!--        </div>-->
      </div>
    </div>
  </q-footer>
</template>
<script setup>
</script>
